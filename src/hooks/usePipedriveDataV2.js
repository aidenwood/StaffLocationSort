// Pipedrive Data Hook V2 - Enhanced Client-Side Filtering
// Fetches all activities for a user and filters for "Property Inspection - [Inspector Name]" client-side

import { useState, useEffect, useCallback, useMemo } from 'react';
import { 
  fetchActivitiesForInspectorV2,
  fetchUserActivities,
...
// Hook for managing Pipedrive V2 data (Filters API) with fallback to mock data
export const usePipedriveDataV2 = (options = {}) => {
  const {
    autoFetch = true,
  ...
  const shouldUseLiveData = useMemo(() => {
    console.log('🔧 PIPEDRIVE V2 DEBUG: Checking if should use live data...');
    ...
    console.log(result ? '✅ V2: Using live Pipedrive data with Filters API' : '❌ No valid Pipedrive IDs - using mock data');
  ...
  const checkHealth = useCallback(async () => {
    ...
    try {
      console.log('🏥 V2: Checking Pipedrive API health...');
      ...
      console.error('❌ V2: Pipedrive health check failed:', error);
  ...
  // Fetch activities with caching - V2 approach using Filters API
  const fetchActivities = useCallback(async (userId = null, startDate = null, endDate = null) => {
    const cacheKey = `v2_activities_${userId}_${startDate}_${endDate}`;
    const cachedData = cache.get(cacheKey);
    
    // Return cached data if it's still valid
    if (cachedData && (Date.now() - cachedData.timestamp) < cacheTimeout) {
      console.log('📋 V2: Using cached activities data');
    ...
    const timeoutId = setTimeout(() => {
      setState(prev => ({ 
        ...prev, 
        isTimeout: true,
        error: 'V2 API request taking longer than expected - possible rate limiting'
      }));
      console.log('⏰ V2 PIPEDRIVE TIMEOUT: Request taking > 3 seconds, likely rate limited');
    }, 3000);
    ...
    try {
      ...
      if (shouldUseLiveData) {
        console.log('🔄 V2 DEBUG: Fetching live Pipedrive data using Filters API...');
        console.log('   userId:', userId);
        console.log('   startDate:', startDate);
        console.log('   endDate:', endDate);
        
        if (userId) {
          // Fetch for specific user only - using V2 Filters API approach
          const pipedriveUser = getAllInspectors().find(inspector => inspector.appId === userId);
          const testUser = getTestUser();
          
          console.log('🔍 V2 DEBUG: User lookup for specific inspector only:');
          ...
          if (pipedriveUser?.id && pipedriveUser?.name) {
            console.log(`📞 V2 FILTERS API: Fetching activities ONLY for ${pipedriveUser.name} (ID: ${pipedriveUser.id})`);
            ...
            // Use V2 approach - fetch all activities and filter client-side
            activities = await fetchActivitiesForInspectorV2(
              pipedriveUser.id, 
              pipedriveUser.name, 
              limitedStartDate, 
              limitedEndDate
            );
            console.log(`✅ V2 FILTERS API: Received ${activities.length} activities for ${pipedriveUser.name}`);
            isLiveData = true;
          } else if (testUser.id && userId === 'test') {
            console.log(`📞 V2 FILTERS API: Fetching activities ONLY for test user ${testUser.name} (ID: ${testUser.id})`);
            ...
            console.log(`✅ V2 FILTERS API: Received ${activities.length} activities for test user`);
            isLiveData = true;
          } else {
            console.warn('⚠️ V2 DEBUG: User not found in configuration, falling back to mock');
          ...
          console.log('📝 V2 DEBUG: No specific user requested - returning empty array for performance');
          console.log('   (Use inspector selector to fetch specific inspector data)');
          activities = [];
          isLiveData = shouldUseLiveData;
        }
      } else {
        console.log('📋 V2: Using mock data (live data disabled)');
      ...
      window.debugActivitiesV2 = activities;
      console.log('🧪 V2 DEBUG: Added', activities.length, 'activities to window.debugActivitiesV2');
      ...
      console.error('❌ V2: Error fetching activities:', error);
      
      // No fallback to mock data - show error state
      console.log('❌ V2 NO FALLBACK: Showing error state instead of mock data');
      ...
      setState(prev => ({
        ...
        loading: false,
        isLiveData: false,
        error: `V2 (Filters): ${error.message}`,
        isTimeout: false
      }));
  ...
  const getInspectorActivities = useCallback(async (inspectorId, date = null) => {
    try {
      ...
      console.error('❌ V2: Error fetching inspector activities:', error);
  ...
  const clearCache = useCallback(() => {
    setCache(new Map());
    console.log('🧹 V2: Pipedrive data cache cleared');
  }, []);
...
// Hook specifically for activity data with date filtering - V2 version
export const usePipedriveActivitiesV2 = (inspectorId = null, date = null) => {
  const {
    activities,
    loading,
    error,
...
export default usePipedriveDataV2;